...
_TERMINAL_STATUSES = frozenset({"APPROVED", "EXECUTING", "PARTIAL", "DONE", "EXPIRED", "REVOKED"})
...
DEPENDENCY_PINS: Mapping[str, str | int] = MappingProxyType(
    {
        ...
        "approval_source_size": 148049,
        "approval_source_sha256": "d0f9f46be82f170c7ae175e53dac767939c6d83cff1e00f8a705cf6ceac8459a",
...
class ActivationVerifier:
    ...
    def verify(self, now: datetime) -> ActivationState:
        ...
        if len(key) < 32 or frozenset(document) != self._FIELDS:
            return ActivationState.inactive("ACTIVATION_FILE_INVALID")
        protected = document.get("protected_authority_ids")
        if (
            ...
            or not protected
            or any(type(item) is not str or not _ID_RE.fullmatch(item) for item in protected)
        ...
        if (
            ...
            or not _SHA256_RE.fullmatch(str(document.get("legacy_gate_receipt_sha256", "")))
        ...
        unsigned = {key_name: value for key_name, value in document.items() if key_name != "mac_sha256"}
        expected = hmac.new(
            key,
        ...
        return ActivationState(
            ...
            protected_authority_ids=tuple(protected),
...
class FleetLeaseCompilerPort:
    """Exact local adapter around the installed public lifecycle methods."""
    ...
    def _verify_dependency_bytes() -> None:
        source = Path.home() / "Code" / "fleet-lease" / "src" / "fleet_lease" / "approvals.py"
        ...
        if (
            len(content) != DEPENDENCY_PINS["approval_source_size"]
        ...
        artifacts = (
            (done / "SAPPHIRE-ORCHESTRATOR-20260728-053-result-r11.md", "task053_result_sha256"),
            (reviews / "SAPPHIRE-ORCHESTRATOR-20260728-053-independent-review-r11.md", "task053_review_sha256"),
            (done / "SAPPHIRE-ORCHESTRATOR-20260728-061-result-r1.md", "task061_result_sha256"),
            (reviews / "SAPPHIRE-ORCHESTRATOR-20260728-061-independent-review-r1.md", "task061_review_sha256"),
            (done / "SAPPHIRE-ORCHESTRATOR-20260728-059-result-r3.md", "task059_result_sha256"),
            (reviews / "SAPPHIRE-ORCHESTRATOR-20260728-059-independent-review-r3.md", "task059_review_sha256"),
        )
        if any(not _artifact_matches(path, str(DEPENDENCY_PINS[key])) for path, key in artifacts):
            raise RailRefused("DEPENDENCY_MISMATCH", 503)

    def _db(self) -> Any:
        if self._database is None:
            self._verify_dependency_bytes()
            try:
                if importlib.metadata.version("fleet-lease") != DEPENDENCY_PINS["fleet_lease_version"]:
                    raise RailRefused("DEPENDENCY_MISMATCH", 503)
                module = importlib.import_module("fleet_lease.approvals")
                if module.BUNDLE_SCHEMA_VERSION != DEPENDENCY_PINS["approval_schema_version"]:
                    raise RailRefused("DEPENDENCY_MISMATCH", 503)
                self._database = module.ApprovalBundleDB(CANONICAL_REGISTRY_PATH)
            except RailRefused:
                raise
            except Exception:
                raise RailRefused("DEPENDENCY_MISMATCH", 503) from None
...
CREATE TABLE IF NOT EXISTS owner_approval_rail_receipts (
...
class OwnerApprovalRail:
        ...
        for action in source.get("actions", []):
            ...
            for parameter in action.get("parameters", []):
                if (
                    ...
                    and parameter.get("name") in {"service", "environment", "account", "destination"}
                    and parameter.get("value") in protected_set
                ):
                    return True
            if any(action.get(field) in protected_set for field in ("environment", "account", "destination")):
                return True
        return False
    ...
    def inspect(self, bundle_id: str) -> dict[str, Any]:
        ...
        elif self._self_modifies(source, activation.protected_authority_ids):
        ...
        safe_actions = [
            {field: action.get(field) for field in action_fields}
        ...
        dto: dict[str, Any] = {
            ...
            "dependency_pins": {
                ...
                "compiler_candidate_tree": DEPENDENCY_PINS[
                    "task053_candidate_tree"
                ],
                "compiler_result_sha256": DEPENDENCY_PINS["task053_result_sha256"],
                "compiler_review_sha256": DEPENDENCY_PINS["task053_review_sha256"],
                "fleet_lease_commit": DEPENDENCY_PINS["task061_merged_commit"],
                "fleet_lease_tree": DEPENDENCY_PINS["task061_merged_tree"],
                "fleet_lease_result_sha256": DEPENDENCY_PINS[
                    "task061_result_sha256"
                ],
                "fleet_lease_review_sha256": DEPENDENCY_PINS[
                    "task061_review_sha256"
                ],
                "fleet_lease_version": DEPENDENCY_PINS["fleet_lease_version"],
                "approval_schema_version": DEPENDENCY_PINS[
                    "approval_schema_version"
                ],
                "approval_source_sha256": DEPENDENCY_PINS[
                    "approval_source_sha256"
                ],
                "consumer_commit": DEPENDENCY_PINS["task059_merged_commit"],
                "consumer_tree": DEPENDENCY_PINS["task059_merged_tree"],
                "consumer_result_sha256": DEPENDENCY_PINS[
                    "task059_result_sha256"
                ],
                "consumer_review_sha256": DEPENDENCY_PINS[
    ...
    def _connection(self) -> sqlite3.Connection:
        connection = self._compiler.approval_connection()
        ...
        with self._connection() as connection:
            ...
            connection.execute(
                ...
                INSERT INTO owner_approval_rail_challenges (
                    ...
                    registry_identity_sha256, issued_at, expires_at, state
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'PENDING')
                """,
                (
                    challenge_hash,
    ...
    def _provision_receipt(
        ...
        *,
        ...
        receipt_sha = canonical_sha256(receipt_content)
        with self._connection() as connection:
            self._ensure_schema(connection)
            connection.execute("BEGIN IMMEDIATE")
            ...
            existing = connection.execute(
                "SELECT * FROM owner_approval_rail_receipts WHERE bundle_sha256 = ?",
            ...
            if existing is None:
                connection.execute(
                    """
                    INSERT INTO owner_approval_rail_receipts (
                        bundle_sha256, challenge_sha256, receipt_sha256,
                        statement_sha256, owner_identity, issued_at, expires_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?)
                    """,
                    expected,
                )
            elif tuple(existing) != expected:
                raise RailRefused("OPEN_RECEIPT_EXISTS")
            ...
            if attended is None:
                connection.execute(
                    """
                    INSERT INTO attended_approval_receipts (
                        receipt_sha256, bundle_sha256, statement_sha256,
                        approver_identity, approver_class, issued_at,
                        expires_at, consumed_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (*attended_expected, None),
                )
            elif tuple(attended)[:7] != attended_expected:
                raise RailRefused("ATTENDED_RECEIPT_MISMATCH")
        return receipt_sha
        ...
        *,
        ...
        if challenge["state"] == "PENDING":
            ...
            if (
                ...
                raise RailRefused("BUNDLE_CHANGED")
        operation_key = "rail-" + hashlib.sha256(
            canonical_json(
                {
    ...
    *,
    ...
    def local_owner(
        ...
        if owner != OWNER_IDENTITY:
            raise HTTPException(status_code=403, detail="owner required", headers=_PRIVATE_HEADERS)
        return owner
    ...
    def mutation_boundary(request: Request) -> None:
        ...
        if content_type != "application/json":
            raise HTTPException(status_code=415, detail="JSON_REQUIRED", headers=_PRIVATE_HEADERS)
        if request.url.scheme != "https" or origin != f"https://{host}":
            raise HTTPException(status_code=403, detail="ORIGIN_INVALID", headers=_PRIVATE_HEADERS)
        if fetch_site is not None and fetch_site != "same-origin":
            raise HTTPException(status_code=403, detail="FETCH_SITE_INVALID", headers=_PRIVATE_HEADERS)
    ...
    async def decide(
        ...
        if session_cookie is None:
            raise HTTPException(status_code=403, detail="SESSION_INVALID", headers=_PRIVATE_HEADERS)
        try:
            result = rail.decide(
                bundle_id,
                owner=owner,
