...
As of this report, with BTC at $65,175, we publish data-backed event probabilities and path forecasts — not guarantees. Average data quality 0.96/1.00. Every claim has a falsifier; execution stays on designated rails under caps and gates.
...
- Trending crypto pulse: CSPR, PONS, AEON.
- — macro context —
...
- Macro: risk-on 30d p=54%; US recession ≤12m p=28%.

## Has Bitcoin bottomed for this phase?

**Probability (as of 2026-07-27T14:13:09Z): 51%** · stance `uncertain` · confidence `low`.
...
| Global risk-on continues over the next 30 days (equities bid, cr… | 54% | 46% | uncertain | low | 0.96 | 30d |
...
- Driver: SPX ~3m: +3.6% (risk appetite)
...
- Driver: Risk backdrop SPX ~3m: +3.6%
...
1. **Observe** — pull public market data.  
2. **Conjecture** — one probability per binary event, with drivers and data quality.  
3. **Path forecast** — short/med/long bands for prices and trends only.  
4. **Score** — resolve events at their resolution window (Brier-style).  
5. **Act (gated)** — trade ideas stay on designated rails under caps.
